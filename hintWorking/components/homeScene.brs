sub init()
    ...
    m.timer = m.top.FindNode("hintTimer")
    ...
    ? "[oWin_scene] init"
     ...
     ' hints for both type of questions
    m.triviaHint = CreateObject("roList")
    m.triviaHint.AddTail("Think about famous actors from California.")
    m.triviaHint.AddTail("It's not Toronto!")
    m.triviaHint.AddTail("This tech giant is based in the Pacific Northwest.")
    m.triviaHint.AddTail("This chain is known for its delivery cars with a light on top.")
    m.triviaHint.AddTail("Look for the symbol on jewelry.")
    m.triviaHint.AddTail("This European country is famous for windmills and tulips.")
    m.triviaHint.AddTail("This pop star is from a Caribbean island.")
    m.triviaHint.AddTail("It's a language spoken in Germany and Austria.")
    m.triviaHint.AddTail("It's less than 60, more than 20.")
    m.triviaHint.AddTail("These animals can live about half a century.")
    m.triviaHint.AddTail("It's not Won or Yuan.")
    m.triviaHint.AddTail("He was a monk who experimented with peas.")
    m.triviaHint.AddTail("He is from India and works for children's rights.")
    m.triviaHint.AddTail("He played for Argentina and Napoli.")
    m.triviaHint.AddTail("It's not a lake in North America.")


    m.tofHint = CreateObject("roList")
    m.tofHint.AddTail("The color of the sky is due to light scattering, not reflection.")
    m.tofHint.AddTail("Humans can survive longer than 3 hours without water.")
    m.tofHint.AddTail("Vending machines cause more accidents than sharks.")
    m.tofHint.AddTail("India is a very linguistically diverse country.")
    m.tofHint.AddTail("Water molecules do break and reform, but not always into H and OH.")
    m.tofHint.AddTail("Check a map for Norway and North Korea's neighbors.")
    m.tofHint.AddTail("The US has hosted the Summer Olympics before LA28.")
    m.tofHint.AddTail("Hamburgers and fries have roots outside the US.")
    m.tofHint.AddTail("Paris is not the city with the most French speakers.")
    m.tofHint.AddTail("Mammoths survived longer than you might think.")
    m.tofHint.AddTail("Cleopatra lived closer to our time than to the pyramids.")
    m.tofHint.AddTail("John Tyler's family tree is surprisingly recent.")
    m.tofHint.AddTail("The US had several European colonizers.")
    m.tofHint.AddTail("Not all kids born on US military bases are automatically citizens.")
    m.tofHint.AddTail("The US has no official language at the federal level.")
...
sub TorF()
    ...
    m.hintLabel.text = "Hint: " + m.tofHint[m.randomNum]
...
sub triviaQuestion()
    ...
    m.hintLabel.text = "Hint: " + m.triviaHint[m.randomInt]
...
sub hintTimer()
    ...
    m.timer.ObserveField("fire", "onHintTimerFire")
