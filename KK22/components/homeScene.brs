...
sub updateAnimalDisplay()
        ...
        if m.turnLabel.text = "Chicken's turn"
            ...
            m.leftAnimal.visible = true
            m.rightAnimal.visible = true
        else if m.turnLabel.text = "Bot's turn (thinking...)"
            m.rightAnimal.uri = "pkg:/images/o.png"
            m.leftAnimal.uri = "pkg:/images/angryChicken.png"  
            m.rightAnimal.visible = true
            m.leftAnimal.visible = true
...
sub onCursorTimeFire()
    ...
    if m.cursorIndex < 9 then
        ...
        'bot stuff, we just want the bot stuff to run after the animation is done'
        checkCondition()
        if m.botMode = 2 and m.botEnabled = 1 and m.cursorTimer.control = "stop"
            ...
            if m.x[position].visible or m.o[position].visible then
                ...
                m.count += 1
                m.botTimer.control = "stop"
                win() 'win() was only called after each player move, this makes sure its called even after the bot moves inspiration: katherine'
            end if 
        end if
    end if
end sub
