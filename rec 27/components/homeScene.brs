sub init()
    ...
    m.questionBankMCQ.AddTail("Which of these is a position solely in American football(other 3 are in soccer too)?")
    m.questionBankMCQ.AddTail("Find the odd one out.")
...
if m.botMode = 2 ' AI MODE POWERED BY AI
        if m.count MOD 2 = 0 then
            m.turnLabel.text = "Bot's turn" ' opposite intuition
            for i = 0 to m.x.Count() - 1
                if m.x[i].visible = false and m.o[i].visible = false then
                    if m.x[i].translation[0] = m.cursor.translation[0] and m.x[i].translation[1] = m.cursor.translation[1] then
                        m.x[i].visible = true
...
sub helperO(pos1, pos2, pos3)
    if m.o[pos1].visible and m.o[pos2].visible and m.o[pos3].visible then
        ...
        m.top.findNode("strikethrough").color = "0xC19A6B"
        m.audio.control = "stop"
    ...
    if selectedContent = m.currentCorrectAnswer then
        ...
        if selectedContent = m.currentCorrectAnswer then
            ...
            if m.count MOD 2 = 0 then
                ...
                m.turnLabel.text = "Bot's turn" ' changed X and O order
                updateAnimalDisplay()
            end if
         end if
    end if 
...
sub showWinWithConfetti()
    ...
    m.strikeThroughAnimation.visible = false
...
sub botTurn()
    ? m.botEnabled
    if m.botMode = 2 and m.botEnabled = 1
    position = rnd(9) - 1
    ? position
    if m.x[position].visible or m.o[position].visible then
        botTurn()

    else 
        m.o[position].visible = true
        m.botEnabled = 0
        m.turnLabel.text = "Player's turn" ' show turn first before system switch
        m.count += 1
